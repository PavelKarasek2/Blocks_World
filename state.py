...
class State:
    ...
    #I changed the name of the first parameter to blocks as that is more descriptive
    @staticmethod
    def find(blocks, id):
        """
        ...
        """
        return next((item for item in blocks
                     if item.id == id), None)

    #Is this not also a static method?
    @staticmethod
    ...
    #creates a square block object in the state
    def square(self, id):
    ...
    #creates a tringle block object in the state
    ...
    #sets a block object with id1 on top a block object with id2 
    def on(self, id1, id2):
        try:
            # retrieve id
